...
# Run a single file
uv run pytest tests/test_orders_service.py

# Run a single test by name
uv run pytest tests/test_orders_service.py::test_create_order_reduces_stock
...
Migrations live in `alembic/versions/` and follow a sequential zero-padded naming convention (`0019_add_promotions.py`, `0020_…`, `0023_…`) — rename the autogenerated hash-prefixed file to the next number and keep the `down_revision` chain linear.
...
```
Router (api/v1/*.py)  →  Pydantic schemas (schemas/*.py)  →  Service (services/*.py)  →  SQLAlchemy models (models/*.py)
```

Routers are thin: parse request body via Pydantic schema, call one service function, return response. Business logic lives exclusively in services — no FastAPI imports there.
...
- **Transaction re-entrancy:** services called *inside* another service's `db.begin()` block must NOT open their own `db.begin()` — nested begins raise an error. Pass the session through and let the outer transaction own the commit.
...
- **Custom errors:** raise from `app/core/errors.py` — `NotFound`, `Unauthorized`, `Forbidden`, `Conflict`, `Unprocessable` — not raw `HTTPException`.
...
| `models/membership.py` | `membership_programs`, `membership_reward_products`, `membership_accounts`, `point_transactions` |
| `models/promotions.py` | `promotions`, `promotion_redemptions` |
| `models/sales.py` | `salespeople` (referenced by `customers.sales_id` FK) |
...
`auth`, `inventory`, `receipts`, `categories`, `products`, `modifier_groups`, `orders`, `pre_orders`, `shopping_list`, `production`, `realtime`, `reports`, `customers`, `hr`, `stock_takes`, `membership`, `promotions`, `salespeople`
...
| `MovementType` | `RECEIVE`, `SALE`, `WASTE`, `ADJUST`, `TRANSFER_IN/OUT`, `PRODUCTION_USE`, `PRODUCTION` |
| `OrderStatus` | `PENDING`, `PAID`, `IN_PROGRESS`, `READY`, `COMPLETED`, `VOID` |
| `Channel` | `DINE_IN`, `TAKEAWAY`, `DELIVERY` |
| `PaymentMethod` | `CASH`, `CARD`, `QR_PROMPTPAY`, `LINE_PAY`, `TRUEMONEY`, `OTHER` |
| `PreOrderStatus` | `PENDING`, `IN_PROGRESS`, `COMPLETED`, `CANCELLED` |
| `FulfillmentMode` | `PRODUCE_FRESH`, `FROM_INVENTORY` |
| `StaffPosition` | `JUNIOR`, `SENIOR`, `HEAD_OF_STAFF` |
| `LeaveType/Status` | `VACATION/SICK/PERSONAL/OTHER`; `PENDING/APPROVED/REJECTED` |
| `TaskStatus` | `TODO`, `IN_PROGRESS`, `PENDING_REVIEW`, `DONE` |
| `ReceiptStatus` | `DRAFT`, `CONFIRMED` |
| `EarnMode` | `PER_RECEIPT`, `PER_BAHT`, `PER_ITEM` |
| `RewardType` | `DISCOUNT_FIXED`, `DISCOUNT_PERCENT`, `FREE_ITEM` |
| `RewardScope` | `ALL`, `CATEGORY`, `SPECIFIC_PRODUCTS` |
| `PointTxType` | `EARN`, `REDEEM`, `ADJUST`, `EXPIRE` |
| `MembershipTier` | `NONE`, `BRONZE`, `SILVER`, `GOLD` |
| `PromotionType` | `PERCENT_OFF`, `COMBO_BUNDLE`, `COMBO_QUANTITY`, `HAPPY_HOUR` |
| `PromotionScope` | `ORDER`, `CATEGORY`, `PRODUCT` |
| `WastageReason` | `EXPIRED`, `SPILLED`, `TRIAL`, `DAMAGED`, `OTHER` |
...
### Alembic: PostgreSQL Enum Types

`sa.Enum(create_type=False)` is silently ignored by `op.create_table` and fires auto-DDL anyway. For new PostgreSQL enum columns, declare the type with `postgresql.ENUM(create_type=False)` and create the type manually via raw SQL before the table:
...
def upgrade() -> None:
    bind = op.get_bind()
    bind.execute(sa.text("CREATE TYPE my_enum AS ENUM ('A', 'B')"))
    op.create_table("my_table", sa.Column("col", my_enum, ...), ...)

def downgrade() -> None:
    op.drop_table("my_table")
    op.get_bind().execute(sa.text("DROP TYPE IF EXISTS my_enum"))
```
